...
const routerFactory = (prisma) => {
  ...
  router.post('/google', async (req, res) => {
    ...
    try {
      ...
      if (emailVerified === false) return res.status(401).json({ error: 'invalid_token' });

      const bySub = await prisma.user.findUnique({ where: { googleSub } });
      const user = bySub
        ? await prisma.user.update({
          where: { id: bySub.id },
          data: {
      ...
      if (!user) return res.status(409).json({ error: 'google_already_linked' });
